...
- **Единый формат записей**: все записи в дневном логе (и факты демонов, и действия кнопок, и логины) содержат PID процесса в `[PID]` — `[monitor]`, `[service]`, `[login.cgi]` и др. единообразно.
- Кэш: `entware.js?v=70`.
