...
- **Логирование доступа к терминалу и процессам**: при открытии вкладок «Терминал» и «Процессы» фронт отправляет событие (`action=log_access` в ttyd_control.cgi), запись `[monitor] Открыта вкладка: терминал/процессы` в суточный лог — уходит в Telegram (источник monitor).
