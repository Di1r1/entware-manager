...
- **Чистка тегов в сообщениях**: `format_message` теперь убирает из текста все известные теги источников (`[service_action]`, `[monitor_action]`, `[ACTION]`, `[view_file.cgi]`, `[login.cgi]` и др.), а не только `[packages]` — сообщения без дублирующих тегов.
- **Разделение логов**: действия панели (Запрос на START/STOP демона, убийство процесса, очистка лога, действия со службами) вынесены из общего дневного лога в отдельные профильные файлы — `/tmp/entware/logs/monitor_actions.log` и `/tmp/entware/logs/service_actions.log`. Дневной суточный лог (`/tmp/entware/logs/YYYY-MM-DD.log`) теперь содержит только фактические события демонов (монитор/сеть/службы), которые идут в Telegram — без дублей действий из панели. В системные логи добавлены источники «Действия защиты» и «Действия служб».
- **Вкладка «Защита» показывает полный лог**: `monitor/log.cgi` теперь объединяет дневной лог (факты демонов) и `monitor_actions.log` (действия панели), сортирует по времени (новые сверху) — полная картина, как раньше. Telegram по-прежнему получает только факты без дублей.
- **Вкладка «Службы» показывает полный лог событий**: `service_watchdog/events.cgi` теперь объединяет дневной лог (факты демона `[service]`) и `service_actions.log` (действия панели `[service_action]` — запуск/остановка ttyd/htop, служб), сортирует по времени. События запуска/остановки ttyd/htop снова видны.
- Кэш: `entware.js?v=68`.
