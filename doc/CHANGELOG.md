...
- **Вставка Ctrl+V в терминале (ttyd) заработала**: xterm.js 5.4 (внутри ttyd 1.7.7) по Ctrl+V шлёт в PTY литеральный `^V` вместо вставки (только `paste`-событие работает). Добавлен форк index.html ttyd (флаг `-I` в `ttyd.go`), который перехватывает Ctrl+V/Cmd+V/Shift+Insert → `term.paste()` (HTTPS через `readText`, HTTP через нативную textarea). Форк попадает в релиз через `build-deploy.sh` (`static/ttyd/index.html`) и chmod в `install.sh`.
- **Вкладка «Пакеты»: пустой список «Установленные» больше не залипает после переустановки/обновления через кнопку.** Причина: пустой массив `[]` (opkg временно недоступен при переустановке, `installed.cgi` отдаёт `[]`) кэшировался в localStorage как валидный на 60 с — следующий вход в вкладку не делал повторный fetch, список оставался пустым до нажатия «Обновить списки пакетов». Теперь: пустой результат не кэшируется и при чтении считается null (всегда refetch); после завершения переустановки/обновления кэш пакетов сбрасывается (`pkgCacheClearAll` при `status=done`); `installed.cgi` дозапрашивается с ретраем (3 попытки × 800 мс) при сетевой ошибке, а успешный ответ (включая пустой) больше не ретраится. Кэш `entware.js?v=27`.
...
- Dev-роутер: `installed.cgi`/`upgradable.cgi` → 200, кэш-версии `entware.js?v=27`/`style.css?v=32` отдаются, HTTP 200; RDP-клипборд откачен к рабочей v1.09.3 (textarea-регрессия устранена).
