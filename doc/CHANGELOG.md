...
- **Логирование входа в терминал и процессы**: событие логируется только при реальной загрузке iframe терминала/htop (терминал запущен и пароль верный), а не при простом открытии вкладки. Запись `[monitor] Открыта вкладка: терминал/процессы` в суточный лог — уходит в Telegram (источник monitor).
