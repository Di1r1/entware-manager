...
- **Перемещение ссылок вверх/вниз.** В настройках (блок «Управление ссылками на главной») у каждой ссылки появились кнопки со стрелками ▲/▼ — можно менять порядок. Порядок сохраняется на сервер (`links_save.cgi`), общий для всех устройств, и сразу применяется на странице статистики (порядок = порядок массива).
- Новые иконки `icon-chevron-up`/`icon-chevron-down` в `icons.svg`.
- Кэш-версии: `entware.js?v=54`, `icons.svg?v=4` (во всех 9 страницах).
