...
for f in "$PROJECT_DIR"/*; do
    name=$(basename "$f")
    case "$name" in
        deploy|go|tmp|build-deploy.sh|TECH_SPEC.md|RULES.md|links.json|DEVLOG.md|DEVICE.md|BUILD.md|router_backup|conffiles|control|postinst|prerm|*_config.json|*.tar.gz|*.ipk)
            continue ;;
    esac
    if [ -d "$f" ]; then
        cp -a "$f" "$DEPLOY_DIR/"
